...
Each section below takes one region of the original, shows the Go it became, and explains the choice. The line numbers refer to the original file, so you can keep it open beside this page. Read the sections in order: later ones assume the earlier ones.
...
Lessons for this region: [Errors are return values, not exceptions](concepts/errors-are-values.md) and [nil is not undef, and nothing autovivifies](concepts/nil-vs-undef.md)
...
Lessons for this region: [strconv turns strings into numbers, and refuses to guess](concepts/strconv-parsing.md), [FindStringSubmatch replaces $1, and no-match returns nil](concepts/submatch-and-named-groups.md), and [bufio.Scanner reads lines, and gives up on long ones](concepts/bufio-scanner-limit.md)
...
Lessons for this region: [Sorting is a function call, and the default is numeric-aware](concepts/sort-slice.md), [Map order is randomised per loop, on purpose](concepts/map-iteration-order.md), and [range gives you the index first, and the element is a copy](concepts/range-is-not-foreach.md)
