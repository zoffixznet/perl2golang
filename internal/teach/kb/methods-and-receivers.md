...
One habit to leave behind entirely: the hand-written accessor. `sub name { $_[0]{name} }` exists because a hash key is not part of any contract and a sub is, so Perl classes wrap every field in one to keep the option of changing it later. Go already has that option: an exported field and a method are spelled the same at the call site, `x.Name` and `x.Name()` differ by two characters, and a field can be replaced by a method later without touching a single caller as long as the name survives. So the Go form of a read-only accessor is an exported field, and the Go form of the read/write pair is the same field assigned to. A getter earns its place only when it computes something, guards something, or is needed to satisfy an interface, because an interface can promise methods and never fields.
