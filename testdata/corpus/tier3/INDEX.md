...
| 30 | `30-error-dispatch` | Error objects thrown, caught in `$@`, and sorted out by asking what they are | a three-level class hierarchy, a `throw` class method, `die $object`, `blessed` and `->isa` dispatch, a base method calling an overridden `label` | 4 | The base method's call on itself has to reach the subclass's version, which embedding alone cannot do |
| 31 | `31-dispatch-table` | A dispatch table of code refs that disagree about their shape | a hash of subs returning a string, a number, a list and nothing, one taking two arguments where the rest take one | 4 | One Go function type cannot hold handlers that disagree, so the table is either uniform and untyped or split by shape |
| 32 | `32-template-method` | A base method calling methods its subclasses replace, which is the shape every report generator is built on | one `render` calling five methods on `$self`, two siblings replacing different subsets, an accessor called from the base, a base object with no subclass in play | 4 | Embedding promotes methods but does not dispatch through them; the fix is an interface plus a back-pointer set by each constructor |
| 33 | `33-dispatch-below-the-root` | The same shape with the dispatching method declared halfway down, which does not convert | a three-level hierarchy dispatching from level two, `SUPER::describe` transformed by its caller, two siblings replacing different steps | 4 | One interface can only cover the hierarchy when the methods are on its root; from the middle there is nothing every class satisfies |
| 34 | `34-named-arguments-as-a-hash` | A constructor that walks `%args` instead of naming its keys, which does not convert | `my ($class, %args) = @_` used as a whole hash, `sort keys %args`, a computed key read, an allow-list filter, a map-valued field | 3 | Named parameters can only be recovered where the keys are written down; here the parameter list is the data |
| 35 | `35-a-grid-behind-a-reference` | The same table one level of indirection away, which does not convert | `$grid->[$r][$c] = ...` on a scalar holding an array reference, the same write inside a sub, `@{ $grid->[$r] }`, a write past the end of both levels | 3 | The growth has to be written back somewhere, and through a reference there is no binding to write it into |
