...
echo
echo "next:"
echo "  1. write the program in $dir/input.pl"
echo "  2. put any arguments in $dir/cmd and any standard input in $dir/stdin"
echo "  3. re-record the expectations:"
echo "       make corpus-record TIER=$tier NAME=$name"
echo "  4. write $dir/notes.md: what it exercises, and what makes it hard"
echo "  5. add a row for it to testdata/corpus/$tier/INDEX.md"
echo "  6. score it: make score ARGS=\"-tier $tier -only $name\""
echo
echo "the manifest row above records no arguments, no stdin and exit status $status."
echo "the scorer reads the entry's own files and says so when the manifest has"
echo "fallen behind, so run it once after editing and read the corpus notes."
